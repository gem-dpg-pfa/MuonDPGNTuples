...
parser.add_argument('--Dataset','-d',nargs='?',choices=['Express', 'Prompt','ZeroBias','ZMu','MinimumBias','RPCMonitor'],help='Dataset to be used (check availability on DAS)',required=True)
parser.add_argument("--Tag", type=str, help="Tag to append to the request name", required=True)
...
if args.Dataset == 'Express':
    ...
    unitsPerJob = 1
    fileSplitting = "FileBased"
elif args.Dataset == 'RPCMonitor':
    globalTag = '140X_dataRun3_Prompt_v4',
    inputDataset = ["/RPCMonitor/Run2024G-v1/RAW"]
    unitsPerJob = 2
    fileSplitting = "FileBased"
...
config.Data.outLFNDirBase = '/store/group/dpg_gem/comm_gem/P5_Commissioning/trigger-performance'
os.makedirs("/eos/cms"+config.Data.outLFNDirBase, exist_ok=True)
...
for run_number in run_list:
    for indataset in inputDataset:
        ...
        crab_folderName = str(run_number)+'_Trigger_'+indataset.split("/")[1]+"_"+args.Tag
        config.Data.inputDataset = indataset
        #config.Data.inputBlocks = ["/RPCMonitor/Run2024I-v1/RAW#eb811c28-f6f4-4ccc-9be2-6cebc9d510d2"]
